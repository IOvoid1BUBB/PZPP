  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok || !payload?.success) {
        setFormError(payload?.error || "Nie udało się wysłać linku resetującego.");
        return;
      }

      setSuccessMessage(
        payload?.message || "Jeśli konto istnieje, wysłaliśmy link resetujący na podany adres."
      );
    } catch (_error) {
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6" noValidate>
            ...
            <div className="space-y-2.5">
              ...
              <Input
                ...
                aria-errormessage={emailError ? "forgot-password-email-error" : undefined}
                disabled={isLoading}
                className="font-medium px-4 py-3"
              />
              {emailError ? (
                <p id="forgot-password-email-error" className="text-red-500 text-sm mt-1">
                  {emailError}
                </p>
              ) : null}
            </div>
