...
export function getSubject({ meetingTitle } = {}) {
  return `Zaproszenie na spotkanie${meetingTitle ? `: ${meetingTitle}` : ""}`;
}

export default function CalendarMeetingInvitation(props) {
  const { meetingTitle, startTime, meetLink } = props || {};
  const formatted = formatDateTimePL(startTime);

  return (
    <Layout preheader="Zaproszenie na spotkanie">
      <Section>
        ...
        {meetLink ? (
          <Section style={{ paddingTop: 18 }}>
            ...
            <Text style={{ fontSize: "12px", lineHeight: "18px", margin: "12px 0 0", color: "#667085" }}>
              Jeśli przycisk nie zadziała, użyj tego linku:{" "}
